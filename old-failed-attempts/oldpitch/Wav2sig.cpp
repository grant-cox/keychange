...
int Wav2sig::convert(std::string filepath, uint8_t column) {
     ...
     //data.push_back(sigvector);
     data.resize(wavHeader.numSamples); //resize to size of data
     ...
     wavHeaders.push_back(wavHeader);

     // for(unsigned int i = 0; i < wavHeader.numSamples; i++) {
     //      //cout << sigvector[i] << " ";
     //      cout << sigvector(i) << " ";
     // }
